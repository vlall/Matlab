%% This an OBJECT script and is 4 Categories on EASY, MEDIUM, and HARD

function [CAT] = objCat1B(sub)
trials = 126;
CAT.curDir = cd; 
...
fprintf('June 2, 2014\n');
pause(.4);
fprintf('Script Written by Vishal Lall\nThis an OBJECT script and is set to 4 Categories on EASY, MEDIUM, and HARD\n\n');
pause(3);
...
%***Timing Variables***
fixTime = 2.0;
% ImageTime Changes in this scipt, see below for difficulty levels
imageTime = .170;
maskTime = .5;
startTime=GetSecs;
...
excel2=fopen([ strcat(subString, '-Obj-Csv-DIFFICULTIES.csv')],'wt');
fprintf(excel2, 'Trial #, Easy, Med, Hard, Total Percent\n');
...
for i = 1:trials
    ...
    %% This Section is for recording the percentage correct on each difficulty.
    if difficulty == 1
    ...
    if difficulty == 1 and choice == category
        easyCorrect = easyCorrect+1;
    elseif difficulty == 2 and choice == category
        medCorrect = medCorrect+1;
    elseif difficulty == 3 and choice == category
        hardCorrect = hardCorrect+1;
    end;  

    easyPercent= (easyCorrect/easyCounter) * 100;
    medPercent= (medCorrect/medCounter) * 100;
    hardPercent= (hardCorrect/hardCounter) * 100;
    fprintf(excel2, '%d, %1.3f\n, %1.3f\n, %1.3f\n, %1.3f\n', i, easyPercent, medPercent, hardPercent, percentCorrect);
